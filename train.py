...
def train():
    ...
    x=tf.placeholder("float", shape=[15, img_rows, img_cols, 3])
    y=tf.placeholder("float", shape=[15, img_rows, img_cols, 1])
    ...
    optimizer = tf.train.AdamOptimizer(learning_rate=lr_init,beta1=0.9).minimize(loss)
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(epochs):
            ...
            for i in range(0,(len(X_train)/batch_size)-1):
                if i==0:
                    xtrain=X_train[i:i+15]
                    ytrain=y_train[i:i+15]
                else:
                    xtrain=X_train[i*15:(i*15)+15]
                    ytrain=y_train[i*15:(i*15)+15]
                c,_= sess.run([loss,optimizer], feed_dict={nn_model.x: xtrain ,nn_model.y:ytrain})
                print ('Iteration: %d  Iteration Loss: %f' %(i,c))
                epoch_loss += c
